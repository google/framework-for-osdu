...
class FileLocationRepositoryImplTest {
  ...
  @BeforeEach
  void setUp() {
    fileLocationRepository = new FileLocationRepositoryImpl(firestore);
  }
  ...
  class FindFileLocation {
    ...
    void shouldFindFileLocationByFileID() {
      ...
      given(firestore.collection(COLLECTION_NAME).whereEqualTo("FileID", FILE_ID).get())
          .willReturn(queryFuture);

      givenDocSnap(qDocSnap, getFileLocation(new Date()));

      // when
      FileLocation fileLocation = fileLocationRepository.findByFileID(FILE_ID);
    ...
    void shouldThrowExceptionWhenQueryFailed() {
      ...
      // when
      Throwable thrown = catchThrowable(() -> fileLocationRepository.findByFileID(FILE_ID));
    ...
    void shouldReturnNullWhenNothingWasFound() {
      ...
      given(firestore.collection(COLLECTION_NAME).whereEqualTo(Fields.FILE_ID, FILE_ID).get())
          .willReturn(queryFuture);
  ...
  class SaveFileLocation {
    ...
    void shouldSaveFileLocationAndReturnSavedEntity() {
      ...
      given(docSnap.getString(Fields.FILE_ID)).willReturn(FILE_ID);
    ...
    void shouldUseServerTimestampWhenCreateAtIsNotSpecified() {
      ...
      then(dataCaptor.getValue()).satisfies(map -> {
        then(map.get(Fields.FILE_ID)).isEqualTo(FILE_ID);
