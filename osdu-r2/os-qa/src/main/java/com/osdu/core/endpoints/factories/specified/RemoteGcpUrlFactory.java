 ...
 */
public class RemoteGcpUrlFactory implements BaseFactory {
    InstanceCreator instanceCreator = new InstanceCreator();

    @Override
    public String getIngest(String resource) {
        return instanceCreator.creator(new IngestCreator()).getUrl(PropertyHolder.remoteProps.getIngestGcpUrl() + resource);
    }

    @Override
    public String getFileService(String resource) {
        return instanceCreator.creator(new FileServiceCreator()).getUrl(PropertyHolder.remoteProps.getFileServiceHost() + resource);
    }

    @Override
    public String getWorkflowService(String resource) {
        return instanceCreator.creator(new FileServiceCreator()).getUrl(PropertyHolder.remoteProps.getWorkflowServiceHost() + resource);
    }
}